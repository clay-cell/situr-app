            ...
            <tbody>
                ...
                @foreach ($datos as $dato)
                    <tr class="hover:bg-gray-100 border-b border-gray-200">
                        ...
                        @endif
                        @if ($titulo == 'Tramites en proceso')
                            <td class="py-3 px-4 text-gray-700">
                                <a href="{{ route('documentos_tramite', $dato->tid) }}"
                                    class="text-sm md:text-sm lg:text-base font-semibold bg-blue-600 hover:bg-blue-800 text-white py-2 px-4 mt-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-100">
                                    <i class="fa-solid fa-eye mr-3"></i> Documentos</a>
                                <a href="{{ route('revisar_tramite.show', [$dato->eid, $dato->tipoTramite]) }}"
                                    class="text-sm md:text-sm lg:text-base font-semibold bg-red-600 hover:bg-red-500 text-white py-2 px-4 mt-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-100">
                                    <i class="fa-solid fa-x mr-3"></i> Rechazar</a>
                            </td>
                        @else
                            @if ($titulo == 'Tramites nuevos ingresados')
                                <td class="py-3 px-4 text-gray-700">
                                    <a href="{{ route('revisar_tramite.show', [$dato->eid, $dato->tipoTramite]) }}"
                                        class="text-sm md:text-sm lg:text-base font-semibold bg-blue-600 hover:bg-blue-800 text-white py-2 px-4 mt-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-100">
                                        <i class="fa-solid fa-eye mr-3"></i> Revision</a>
                                    <a href="{{ route('revisar_tramite.show', [$dato->eid, $dato->tipoTramite]) }}"
                                        class="text-sm md:text-sm lg:text-base font-semibold bg-red-600 hover:bg-red-500 text-white py-2 px-4 mt-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-100">
                                        <i class="fa-solid fa-x mr-3"></i> Rechazar</a>
                                </td>
